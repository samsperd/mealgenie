        ...
        <div class="px-3 py-2">
            ...
            <div v-else>
                <div v-for="(item, indice) in cart" :key="indice" class="">
                    <div class="col-12 p-0 border-0 clearfix"><dt class="float-left"><small class="btn" @click="removal(indice)"><i class="zmdi zmdi-close-circle text-danger"></i></small>{{ item.name }}</dt> <dt class="float-right pt-1"><small>{{ item.quantity }} x</small> &#8358;{{ item.original }}</dt></div>
                    ...
                    <small><b class="text-dark" v-if="item.type == 0">{{item.quantity}} x {{item.pack.name}} &#8358;{{item.pack.price}}</b>
                    <b v-else>+ {{item.pack.name}} &#8358;{{item.pack.price}}</b>
                    </small>
                    <h6 class="p-1">Total + Pack and Delivery: <b class="text-success float-right">&#8358;{{item.price}}</b></h6>
                    <hr>
                </div>
